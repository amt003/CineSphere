...
export default function AddMovie() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      showError("Please fill in all required fields including poster image");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        showError("A movie with this title already exists");
      } else if (response.status === 403) {
        showError("You don't have permission to add movies");
      } else {
        showError(data.message || "Failed to add movie");
      }
    } catch (err) {
      console.error("Error adding movie:", err);
      showError("Error adding movie. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 py-8">
          <div className="max-w-3xl mx-auto px-4">
            ...
            <p className="text-slate-400 mt-2">
              Add a movie to your theatre's collection
            </p>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <label htmlFor="isActive" className="text-sm font-semibold">
                  Make this movie active and available for bookings
                </label>
              </div>
